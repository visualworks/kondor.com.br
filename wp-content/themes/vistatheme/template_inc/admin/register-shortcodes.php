...
if(!function_exists('themeple_sc_header_title')){
    ...
    function themeple_sc_header_title($atts, $content=null, $shortcodename =""){
        ...
        switch ($header_size) {
            case 'h6':
                $output .= '<h6>'.$title.'</h6><span class="colore_div"></span>';
                break;
            case 'h4':
                $output .= '<h4>'.$title.'</h4>';
                break;
    ...
    function themeple_lightbox($atts, $content = null, $shortcodename=""){
        ...
        extract(shortcode_atts(array("image_link" => "", 'video' => ''), $atts));
            ...
            if($video != ''){
                    ...
                    $output .= '<a class="lightbox-media1" href="'.$video.'">';
...
if(!function_exists('themeple_list')){
    function themeple_list( $atts, $content = null, $shortcodename="" ) {
        extract( shortcode_atts( array('style' => 'check'), $atts ) ); 
        if($style == 'number')
            $output = '<ol class="default_list '.$style.'">';
        else
            $output  = '<ul class="default_list '.$style.'" >';
...
if(!function_exists('themeple_counter')){
    ...
    function themeple_counter($atts, $content = null, $shortcodename=""){
         ...
         extract(shortcode_atts(array('number' => 50, 'text' => 'Default', 'speed' => 2000, 'color' => '#828284'), $atts));
         $output = '<div class="count_to animate_onoffset">';
            $output .= '<span class="timer" style="color:'.$color.';" data-from="0" data-to="'.$number.'" data-speed="'.$speed.'"></span>';
            $output .= '<span class="text" style="color:'.$color.';">'.$text.'</span>';
         $output .= '</div>';
         return $output;
...
if(!function_exists('themeple_social_subscribers')){
    function themeple_social_subscribers($atts, $content = null, $shortcodename = ""){
        
        extract(shortcode_atts(array('twitter' => 'yes', 'facebook' => 'yes', 'youtube' => 'yes'), $atts));
        $output = '<div class="social_subscribers">';
        if(function_exists('stcounter')){
            if($facebook == 'yes'){
                $output .= '<div class="facebook"><span class="img"></span><div class="data"><span class="title">'.__('Fans', 'themeple').'</span><span class="counter">'.stcounter('facebook').'</span></div></div>';
            }
            if($twitter == 'yes'){
                $output .= '<div class="twitter"><span class="img"></span><div class="data"><span class="title">'.__('Followers', 'themeple').'</span><span class="counter">'.stcounter('twitter').'</span></div></div>';
            }
            if($youtube == 'yes'){
                $output .= '<div class="youtube"><span class="img"></span><div class="data"><span class="title">'.__('Subscribers', 'themeple').'</span><span class="counter">'.stcounter('youtube').'</span></div></div>';
            }
            
        }else{
            $output .= 'Please install before the recommanded plugin "Subscribers Text Counter" ';
        }
        $output .= '</div>';
        return $output;

    }
    add_shortcode('social_subscribers', 'themeple_social_subscribers');
}
