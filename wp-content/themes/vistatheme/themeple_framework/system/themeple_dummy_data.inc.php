...
class themeple_dummy_data extends WP_Import{
	...
	{	
		...
		if(is_array($options))
		{
			...
			{
				$database_option[$page['parent']] = $this->get_default($options[$page['parent']], $page, $controller->subs);
			}
		}
		if(!empty($database_option))
		{
			
			update_option($controller->db_options_prefix, $database_option);
		}
		if(!empty($dynamic_pages))
		{
			update_option($controller->db_options_prefix.'_dynamic_pages', $dynamic_pages);
		}
		
		if(!empty($dynamic_elements))
		{
			update_option($controller->db_options_prefix.'_dynamic_elements', $dynamic_elements);
		}
	}
