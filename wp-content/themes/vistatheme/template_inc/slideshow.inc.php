...
class themeple_slideshow{
    ...
    function display($force_display = false){
        ...
        elseif($this->slide_type == 'news_slider')
            return $this->news_slider();
        elseif($this->slide_type == 'featured_news')
    ...
    function news_slider(){
        ...
        query_posts($args);
        $this->slide_container_class = 'flexslider news_slider';
        $this->slide_ul_class = "slides"; 
        $this->slide_container_id = 'content';
        $this->img_size = 'blog_image';
        
        while(have_posts() ): the_post();

        $this->slides[] = array('slideshow_image' => get_post_thumbnail_id(get_the_ID()), 'slideshow_description' => get_the_title());
    ...
    function featured_news(){
        $big_featured = themeple_post_meta($this->post_id, 'big_featured');
        $small_box_1 = themeple_post_meta($this->post_id, 'small_box_1');
        $small_box_2 = themeple_post_meta($this->post_id, 'small_box_2');
        $output = '<div class="featured_news row-fluid">';
        $this->slide_number = 5;
        $output  .= '<div class="big_featured span9"><img src="'.themeple_image_by_id(get_post_thumbnail_id($big_featured), 'blog_image', 'url').'" alt="" /><div class="captionss"><p class="flex-caption"><span>'.get_the_title($big_featured).'</span></p></div></div>';

      


        $output .= '<div class="others span3">';
        

            $output  .= '<div class="small_featured"><img src="'.themeple_image_by_id(get_post_thumbnail_id($small_box_1), 'small_featured', 'url').'" alt="" /><div class="captionss"><p class="flex-caption"><span>'.get_the_title($small_box_1).'</span></p></div></div>';

            $output  .= '<div class="small_featured"><img src="'.themeple_image_by_id(get_post_thumbnail_id($small_box_2), 'small_featured', 'url').'" alt="" /><div class="captionss"><p class="flex-caption"><span>'.get_the_title($small_box_2).'</span></p></div></div>';
